...
private:
    ...
    public:
        ...
        Data ProcessPartial(const NodeHolder& node, IndexSegment, const Data& left_result, const Data& right_result) const {
...
public:
    ...
    BudgetManager() : tree_(make_unique<SummingSegmentTree<MoneyState, BulkLinearUpdater>>(DAY_COUNT)) {}
...
struct AddMoneyRequest : ModifyRequest {
    ...
    void Process(BudgetManager& manager) const override {
        ...
        const MoneyState daily_change = SIGN == 1 ? MoneyState{daily_value, 0} : MoneyState{0, daily_value};
