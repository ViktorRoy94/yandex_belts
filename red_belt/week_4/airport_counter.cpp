...
public:
    ...
    size_t Get(TAirport airport) const {
        size_t airport_index = static_cast<size_t>(airport);
        return data_[airport_index];
    }
    ...
    void EraseAll(TAirport airport) {
        size_t airport_index = static_cast<size_t>(airport);
        data_[airport_index] = 0;
    }

    static const size_t AIRPORT_COUNT = static_cast<size_t>(TAirport::Last_);
    using Item = pair<TAirport, size_t>;
    using Items = array<Item, AIRPORT_COUNT>;
    ...
    Items GetItems() const {
        Items result;
        for (int airport_idx = 0; airport_idx < AIRPORT_COUNT; airport_idx++) {
            result[airport_idx] = {static_cast<TAirport>(airport_idx), data_[airport_idx]};
        }
        return result;
    }

private:
    array<size_t, AIRPORT_COUNT> data_;
};
