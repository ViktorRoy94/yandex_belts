...
std::vector<RequestPtr> ReadJsonRequests(std::istream& in_stream)
{
    vector<RequestPtr> requests;

    Document doc = Load(in_stream) ;
    vector<Node> json_update_requests = doc.GetRoot().AsMap().at("base_requests").AsArray();
    ...
    vector<Node> json_read_requests = doc.GetRoot().AsMap().at("stat_requests").AsArray();
    requests.reserve(json_read_requests.size());
