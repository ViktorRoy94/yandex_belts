...
struct MoneyState {
    ...
    void ApplyTax(double tax_factor) {
        earned *= tax_factor;
    }
...
public:
    double ComputeIncome(const Date& from, const Date& to) const {
        ...
        MoneyState state = accumulate(data_.begin() + begin_idx,
                                      data_.begin() + end_idx,
                                      MoneyState());
        return state.ComputeIncome();
    }
    ...
    void Earn(const Date& from, const Date& to, int value) {
        ...
        for (auto i = begin_idx; i < end_idx; i++) {
            data_[i].earned += value_chunk;
        }
    }
    void PayTax(const Date& from, const Date& to, int percantage = 13) {
        int begin_idx = DayIndex(from, ZERO_DATE);
        int end_idx = DayIndex(to, ZERO_DATE) + 1;
        double factor = (100 - percantage) / 100.0;
        for (auto i = begin_idx; i < end_idx; i++) {
            data_[i].ApplyTax(factor);
        }
    }
...
private:
    ...
    array<MoneyState, MAX_DAYS_COUNT> data_;
};
...
struct Query
{
    ...
    optional<int> percantage = nullopt;
};
...
Query ParseQuery(const string& str) {
    ...
    if (command == "PayTax") {
        int p;
        query_stream >> p;
        q.percantage = optional<double>(p);
    }
...
ostream& operator<<(ostream& out_stream, const Query& q) {
    ...
    if (q.percantage.has_value()) {
        out_stream << q.percantage.value() << " ";
    }
    return out_stream;
}
...
vector<double> ProcessQueries(const vector<Query>& queries) {
    ...
    for (const auto& q : queries) {
        if (q.command == CommandType::ComputeIncome) {
            ...
            budget.PayTax(q.from, q.to, q.percantage.value());
        } else if (q.command == CommandType::Spend) {
            budget.Spend(q.from, q.to, q.value.value());
        }
    }
    return total_income;
}
...
void TestParseQuery() {
    {
        ...
        ASSERT_EQUAL(q.percantage.has_value(), false);
    }
    {
    ...
    {
        Query q = ParseQuery("PayTax 2000-01-02 2000-01-03 13");
        ASSERT_EQUAL(q.command, CommandType::PayTax);
        ASSERT_EQUAL(q.from, Date({2000, 1, 2}));
        ASSERT_EQUAL(q.to, Date({2000, 1, 3}));
        ASSERT_EQUAL(q.value.has_value(), false);
        ASSERT_EQUAL(q.percantage.has_value(), true);
        ASSERT_EQUAL(q.percantage.value(), 13);
    }
}
...
void TestReadQueries() {
    ...
    const vector<Query> result = {
        ...
        Query({CommandType::PayTax, Date({2000, 1, 2}), Date({2000, 1, 3}), nullopt, 13}),
        Query({CommandType::ComputeIncome, Date({2000, 1, 1}), Date({2001, 1, 1})}),
        Query({CommandType::Earn, Date({2000, 1, 3}), Date({2000, 1, 3}), 10}),
        Query({CommandType::ComputeIncome, Date({2000, 1, 1}), Date({2001, 1, 1})}),
        Query({CommandType::PayTax, Date({2000, 1, 3}), Date({2000, 1, 3}), nullopt, 13}),
        Query({CommandType::ComputeIncome, Date({2000, 1, 1}), Date({2001, 1, 1})}),
        };
...
void Test14() {
    {
        istringstream input("12\n"
                            ...
                            "ComputeIncome 2000-01-01 2001-01-01\n");
        const auto requests = ReadQueries(input);
        const auto responses = ProcessQueries(requests);
        ASSERT_EQUAL(responses, vector<double>({100., 90., 54., 154., 144., 144.}));
    }
    {
        istringstream input("8\n"
        ...
        const auto requests = ReadQueries(input);
        const auto responses = ProcessQueries(requests);
        ASSERT_EQUAL(responses, vector<double>({100, 50, 40, 36}));
    }
    {
        ...
        const auto requests = ReadQueries(input);
        const auto responses = ProcessQueries(requests);
        ASSERT_EQUAL(responses, vector<double>({100, 0}));
    }
}
