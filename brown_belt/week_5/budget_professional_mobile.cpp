...
struct MoneyState {
    ...
    MoneyState ApplyTax(double tax_factor) {
        return {earned * tax_factor, spent};
    }
...
struct BulkTaxApplier {
    ...
    BulkTaxApplier()
    {}
    BulkTaxApplier(int percentage) : factor(1.0 - percentage / 100.0)
    {}

    void AddTax(const BulkTaxApplier& other) {
        factor *= other.factor;
    }
};
...
struct PayTaxRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        tax_percantage = ConvertToInt(input);
    }

    void Process(BudgetManager& manager) const override {
        manager.AddBulkOperation(MakeDateSegment(date_from, date_to), BulkTaxApplier(tax_percantage));
    }

    Date date_from = START_DATE;
    Date date_to = START_DATE;
    int tax_percantage = 0;
};
...
struct SpendRequest : ModifyRequest {
    ...
    void ParseFrom(string_view input) override {
        date_from = Date::FromString(ReadToken(input));
        date_to = Date::FromString(ReadToken(input));
        loss = ConvertToInt(input);
    }

    void Process(BudgetManager& manager) const override {
        const auto date_segment = MakeDateSegment(date_from, date_to);
        const double daily_loss = loss * 1.0 / date_segment.length();
        manager.AddBulkOperation(date_segment, BulkMoneyAdder{MoneyState{0, daily_loss}});
    }
...
void TestParseDate() {
    ASSERT_EQUAL(ComputeDayIndex(Date::FromString("2000-01-01")), 0u);
    ASSERT_EQUAL(ComputeDayIndex(Date::FromString("2000-01-02")), 1u);
}
