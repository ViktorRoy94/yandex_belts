...
void TestReadDomains() {
    ...
    vector<Domain> result = {Domain("ya.ru"),
                             Domain("maps.me"),
                             Domain("m.ya.ru"),
                             Domain("com")};
    const vector<Domain> domains = ReadDomains(input);
    ASSERT_EQUAL(domains, result);
}

void TestGetReverseParts() {
    Domain d("1.2.3.4.5.6.ru");
    const auto reversed_parts_range = d.GetReversedParts();
    vector<string> reversed_parts(begin(reversed_parts_range), end(reversed_parts_range));
    vector<string> result = {"ru", "6", "5", "4", "3", "2", "1"};
    ASSERT_EQUAL(reversed_parts, result);
}

void TestIsSubdomain() {
    ASSERT(IsSubdomain(Domain("maps.me"), Domain("maps.me")));
    ASSERT(IsSubdomain(Domain("m.maps.me"), Domain("maps.me")));
    ASSERT(IsSubdomain(Domain("m.ya.ru"), Domain("ya.ru")));
    ASSERT(IsSubdomain(Domain("moscow.m.ya.ru"), Domain("ya.ru")));
    ASSERT(IsSubdomain(Domain("ya.com"), Domain("com")));
    ASSERT(IsSubdomain(Domain("ha.ya.ya"), Domain("ya.ya")));
    ASSERT(IsSubdomain(Domain("d.b.c"), Domain("b.c")));
    ASSERT(!IsSubdomain(Domain("haya.ya"), Domain("ya.ya")));
    ASSERT(!IsSubdomain(Domain("ya.ya"), Domain("ya.ru")));
    ASSERT(!IsSubdomain(Domain("com.ru"), Domain("com")));
    ASSERT(!IsSubdomain(Domain("ru.com.en"), Domain("com")));
    ASSERT(!IsSubdomain(Domain("vk.com"), Domain("m.vk.com")));
}

void TestIsDomainBanned() {
    {
        istringstream banned_input("4\n"
                                   "ya.ru\n"
                                   "maps.me\n"
                                   "m.ya.ru\n"
                                   "com"
        );
        istringstream to_check_input("7\n"
                                     "ya.ru\n"
                                     "ya.com\n"
                                     "m.maps.me\n"
                                     "moscow.m.ya.ru\n"
        ...
        const vector<Domain> banned_domains = ReadDomains(banned_input);
        const vector<Domain> domains_to_check = ReadDomains(to_check_input);
        vector<bool> check_results = CheckDomains(banned_domains, domains_to_check);
        vector<bool> result = {false, false, false, false, false, true, true};
        ASSERT_EQUAL(check_results, result);
    }
    {
        istringstream banned_input("2\n"
                                   "b.c\n"
                                   "a.b.c\n"
        );
        istringstream to_check_input("1\n"
                                     "d.b.c\n"
        );
        ...
        const vector<Domain> domains_to_check = ReadDomains(to_check_input);
        vector<bool> check_results = CheckDomains(banned_domains, domains_to_check);
        ASSERT_EQUAL(check_results, vector<bool>({false}));
    }
}

void TestPrintBannedDomains() {
    ostringstream output;
    PrintCheckResults(vector<bool>({false, true}), output);
...
int main() {
    ...
    RUN_TEST(tr, TestGetReverseParts);
