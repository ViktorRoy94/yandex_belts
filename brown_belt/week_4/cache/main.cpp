...
void TestCaching(const Library& lib) {
    ...
    // самостоятельно.
    ICache::BookPtr book1 = cache->GetBook(lib.book_names[0]);
    ICache::BookPtr book2 = cache->GetBook(lib.book_names[0]);
    ICache::BookPtr book3 = cache->GetBook(lib.book_names[0]);
    ASSERT_EQUAL(unpacker->GetUnpackedBooksCount(), 1);
    ASSERT_EQUAL(book1, book2);
    ASSERT_EQUAL(book2, book3);
}
...
void TestCaching2(const Library& lib) {
    auto unpacker = make_shared<BooksUnpacker>();
    ICache::Settings settings;
    settings.max_memory = lib.content.at(lib.book_names[0])->GetContent().size() +
                          lib.content.at(lib.book_names[1])->GetContent().size() +
                          lib.content.at(lib.book_names[2])->GetContent().size();
    auto cache = MakeCache(unpacker, settings);

    ICache::BookPtr book1 = cache->GetBook(lib.book_names[0]);
    ICache::BookPtr book2 = cache->GetBook(lib.book_names[1]);
    ICache::BookPtr book3 = cache->GetBook(lib.book_names[2]);
    ICache::BookPtr book4 = cache->GetBook(lib.book_names[0]);
    ICache::BookPtr book5 = cache->GetBook(lib.book_names[1]);
    ICache::BookPtr book6 = cache->GetBook(lib.book_names[0]);
    ASSERT_EQUAL(unpacker->GetUnpackedBooksCount(), 3);
//    ASSERT_EQUAL(book1, book2);
//    ASSERT_EQUAL(book2, book3);
    ASSERT_EQUAL(book1, book4);
    ASSERT_EQUAL(book1, book6);
    ASSERT_EQUAL(book2, book5);
}
