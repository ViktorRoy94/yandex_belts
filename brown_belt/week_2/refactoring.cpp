...
void TestXmlToJson() {
    ...
    Xml::Document xml_doc_after_json = JsonToXml(json_doc, "july");

    const vector<Xml::Node>& xml_doc_childrens = xml_doc.GetRoot().Children();
    const vector<Xml::Node>& xml_doc_after_json_childrens = xml_doc_after_json.GetRoot().Children();

    for (size_t i = 0; i < xml_doc_childrens.size(); ++i) {
        const string feedback_msg = "i = " + std::to_string(i);
        const Xml::Node& c1 = xml_doc_childrens[i];
        const Xml::Node& c2 = xml_doc_after_json_childrens[i];

        AssertEqual(c1.Name(), c2.Name(), feedback_msg);
        AssertEqual(c1.AttributeValue<string>("category"), c2.AttributeValue<string>("category"),
                    feedback_msg);
        AssertEqual(c1.AttributeValue<int>("amount"), c2.AttributeValue<int>("amount"),
                    feedback_msg);
    }
}
...
void TestJsonToXml() {
    ...
    const Document json_doc_after_xml = XmlToJson(xml_doc);
    const vector<Node>& json_doc_array = json_doc.GetRoot().AsArray();
    const vector<Node>& json_doc_after_xml_array = json_doc_after_xml.GetRoot().AsArray();

    AssertEqual(json_doc_array.size(), json_doc_after_xml_array.size());
    for (size_t i = 0; i < json_doc_array.size(); i++) {
        AssertEqual(json_doc_array[0].AsString(),
                json_doc_after_xml_array[0].AsString());
    }
}
